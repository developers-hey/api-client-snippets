...
1. CERT_PATH: This key specifies the path to the mTLS certificate file file in the APIConsumer project. `Spanish translation:` Esta clave especifica la ruta al archivo del certificado mTLS en el proyecto APIConsumer  
2. PRIVATE_KEY_PATH= This key specifies the path to the client private key file in the APIConsumer project. `Spanish translation:` Esta clave especifica la ruta al fichero de la clave privada del cliente en el proyecto APIConsumer. 
3. PUBLIC_KEY_PATH: This key specifies the path to the server public key file in the APIConsumer project. `Spanish translation:` Esta clave especifica la ruta al fichero de clave pública del servidor en el proyecto APIConsumer. 
4. HOSTNAME: Defines the hostname of the remote server. `Spanish translation:` Define el nombre de host del servidor remoto.
...
6. OAUTH_CLIENT_SECRET: The client secret used to authenticate with the remote server. `Spanish translation:` El secreto de cliente utilizado para autenticarse con el servidor remoto.
7. B_APPLICATION: Unique consumer application identifier, used to sign and encrypt the payload. `Spanish translation:`  Identificador único de la aplicación del consumidor, utilizado para firmar y cifrar la carga útil.



## Create PEM Certificates

Extract the security kit in the path, it is recommended to use the resources path of this project. Then I ran the following commands to convert the certificates sent to pem.  `Spanish translation:` Extraiga el kit de seguridad en la ruta, se recomienda utilizar la ruta de recursos de este proyecto. Luego ejecute los siguientes comandos para convertir los certificados enviados a pem.
...
## Step 2: Define la funcion for signing and encrypting the payload. `Spanish translation:` Paso 2: Define la función para firmar y encriptar el payload.

To sign and encrypt a payload, use the signAndEncryptPayload(requestPayload) function. This function takes the payload to be signed and encrypted as an argument and returns the signed and encrypted payload as a string. `Spanish translation:` Para firmar y cifrar una carga útil, utilice la función signAndEncryptPayload(requestPayload). Esta función toma la carga útil que se va a firmar y cifrar como argumento y devuelve la carga útil firmada y cifrada como una cadena.
...
```
## Step 3:  Define la funcion for Decrypt and verify signed payload `Spanish translation:` Paso 3: Define la función para Descifrar y verificar la carga firmada

To decrypt and verify the signature of a JWE/JWS payload, use the decryptAndVerifySignPayload(responsePayload) function. This function takes the JWE/JWS payload to be decrypted and verified as an argument and returns the decrypted and verified payload as a string. `Spanish translation:`
```javascript
...
```
## Step 4:  Export the funcion created `Spanish translation:` Paso 3: Exporta las funciónes creadas,
```javascript
module.exports =
{
...
# APIClient Module  `Spanish translation:` 

## Description
This module exports the APIClient class, which provides a simple way to make HTTP requests to an API using OAuth 2.0 authentication with client credentials grant flow and encrypted payloads. `Spanish translation:`
...
Create the ApiClient class  where the SecurityManager class is defined. Then, declare the following attributes. `Spanish translation:` Cree la clase ApiClient donde se define la clase SecurityManager. Luego, declara los siguientes atributos.
...
### 1.1 Create constructs a ApiClient object with the specified parameters. `Spanish translation:` Crea un constructor ApiClient con los parámetros
...
```
## Step 2 Define the method getAuthorizationToken `Spanish translation:` 
...
```
## Step 3: Define the makeRequest method. `Spanish translation:`  Definir el método makeRequest.
...
```
### Step 1:  Crea una instancia del cliente APIClient, que se utilizará para realizar solicitudes a un servidor. ` Spanish translation:`

```javascript
const client = new APIClient();
```

 ### Step 2: Defines the endpoint, HTTP method and payload to be sent with the request. ` Spanish translation:` Paso 2: DDefine el punto final, el método HTTP y la carga útil que se enviará con la solicitud. 
 ```javascript
...
const requestPayload = `{"taxRegimeId": 2,"name": "Jose Luis","lastName": "Lemus","secondLastName": "Valdivia","businessName": "","birthday": "1996-10-03","rfc": "LEVL961003KQ0","curp": "LEVL961003HBSMLS06","callingCode": "52","cellPhoneNumber": "3311065681","email": "jose.lemus@banregio.com","nationalityId": "001","countryId": "01","stateId": "047","cityId": "04701005","legalRepresentative": {"name": "","lastName": "","secondLastName": ""}}`;
 ```
### Step 3: Define the application headers ` Spanish translation:` Paso 3: Defina las cabeceras de la aplicación
 ```javascript
const headers = {
...
client.getAuthorizationToken().then((accessToken) => {
 ...
 ```
### Step 5: Make the API request with the signed and encrypted payload. And Print the response. ` Spanish translation:` Paso 5: Realizar la petición API con la carga útil firmada y encriptada. E Imprime la respuesta.
 ```javascript
client.getAuthorizationToken().then((accessToken) => {
